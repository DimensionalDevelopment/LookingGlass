...
public class CameraAnimatorPlayer implements ICameraAnimator {
	...
	private void updateTargetPosition() {
		...
		if (!camera.chunkExists(x, z)) {
			if (this.camera.getBlockData().getBlockState(new BlockPos(x, y, z)).getBlock().isPassable(this.camera.getBlockData(), new BlockPos(x, y, z))) {
				while (y > 0 && this.camera.getBlockData().getBlockState(new BlockPos(x, --y, z)).getBlock().isPassable(this.camera.getBlockData(), new BlockPos(x, y, z)))
					;
				if (y == 0) y = target.getY();
				else y += 2;
			} else {
				while (y < 256 && !this.camera.getBlockData().getBlockState(new BlockPos(x, ++y, z)).getBlock().isPassable(this.camera.getBlockData(), new BlockPos(x, y, z)))
					;
