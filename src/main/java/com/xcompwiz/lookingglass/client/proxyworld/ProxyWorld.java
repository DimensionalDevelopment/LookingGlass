...
public class ProxyWorld extends WorldClient {
    ...
    public void makeFireworks(double x, double y, double z, double motionX, double motionY, double motionZ, NBTTagCompound compound) {
        for (WorldView activeview : ProxyWorldManager.getWorldViews(provider.getDimension())) {
            activeview.getEffectRenderer().addEffect(new ParticleFirework.Starter(this, x, y, z, motionX, motionY, motionZ, activeview.getEffectRenderer(), compound));
        }
    }
}
