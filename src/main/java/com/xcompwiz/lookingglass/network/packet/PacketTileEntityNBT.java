...
public class PacketTileEntityNBT extends PacketHandlerBase {
    ...
    public void handle(PacketBuffer data, EntityPlayer player) {
        ...
        if (!proxyworld.getBlockState(new BlockPos(xPos, yPos, zPos)).getBlock().equals(Blocks.AIR)) {
            TileEntity tileentity = proxyworld.getTileEntity(new BlockPos(xPos, yPos, zPos));
            ...
            proxyworld.markChunkDirty(new BlockPos(xPos, yPos, zPos), tileentity);
            proxyworld.setTileEntity(new BlockPos(xPos, yPos, zPos), tileentity);
            //proxyworld.markBlockForUpdate(xPos, yPos, zPos); // TODO!!!
        }
    }
}
