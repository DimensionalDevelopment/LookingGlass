...
public class ChunkFinder {
	 ...
	 */
	public ChunkFinder(BlockPos root, int dimension, IChunkProvider chunkProvider, EntityPlayerMP player, int range) {
				...
				for (int i = minStep; i < maxStep; i++) {
					for (int j = minStep; j < maxStep; j++) {
						...
						if (map[i][j].doAdd() && dist < range2 && dist >= range3) {
							...
							LookingGlass.instance.INSTANCE.sendTo(new PacketChunkInfo(c2, dimension), player);
						}
					}
				}
			}
	...
	public static boolean isBlockNormalCubeDefault(Chunk chunk, int par1, int par2, int par3, boolean par4) {
		if (par1 >= -30000000 && par3 >= -30000000 && par1 < 30000000 && par3 < 30000000) {
			if (chunk != null && !chunk.isEmpty()) {
				return chunk.getBlockState(new BlockPos(par1 & 15, par2, par3 & 15)).isNormalCube();
			}
		}
		return par4;
	}
